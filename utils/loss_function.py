...
def binary_shadow_to_image(shadowless, binary_estimated_shadow, shadow_prob_theshold=0.5):
    binary_estimated_shadow = F.softmax(binary_estimated_shadow, dim=1)
    binary_estimated_shadow = (binary_estimated_shadow > shadow_prob_theshold).float()
    shadow = binary_estimated_shadow[:,1,:,:].unsqueeze(1)
    shadow = shadow.expand(-1, 3, -1, -1)*0.45
    return shadowless*(1 - shadow)
                        
# Reconstruction + KL divergence losses summed over all elements and batch
def vae_loss_function(shadowless, binary_estimated_shadow, true_shadowed_image, mu, logvar):
